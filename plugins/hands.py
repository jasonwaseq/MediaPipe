"""
MediaPipe Hands plugin using the Tasks API (HandLandmarker).
"""
...
class HandsPlugin(MediaPipePluginBase):
    plugin_id = "hands"
    display_name = "Hands"
    ...
    def init(self, settings: dict[str, Any]) -> None:
        self.close()
        model_path = str(get_model_path("hand_landmarker.task"))
    ...
    def process(
        ...
        if result.handedness:
            for cats in result.handedness:
                if cats:
                    c = cats[0]
                    detections.append(
                        {"label": c.category_name, "score": c.score}
                    )
